    ...
    .then(() => {
        ...
        Seance.find({ date_debut: { $lt: date }, date_fin: { $gt: date }, isOptionnel: false }).populate('matiere_id').populate('formateur_id').then(seances => {
            ...
            Presence.find({ seance_id: { $in: seanceIds }, isPresent: true }).then(presences => {
                ...
                Etudiant.find({ classe_id: { $in: listIds }, user_id: { $nin: userIds }, valided_by_support: true }).populate('user_id').populate({ path: "classe_id", populate: { path: 'diplome_id' } }).then(etudiants => {
                    etudiants.forEach(etu => {
                        if (etu.user_id && etu.classe_id.diplome_id.domaine == "Informatique")
                    ...
                    //TODO Récupérer le nombre d'absences dans le mois
                    let htmlmail = `
                    ...
                    let attachments = [{
                        filename: 'signature_igpinformatique.png',
                        path: '/home/ubuntu/ems3/back/assets/signature_igpinformatique.png',
