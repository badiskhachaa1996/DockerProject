...
@Component({
  selector: 'app-gen-doc-paiement-preinscription',
  templateUrl: './gen-doc-paiement-preinscription.component.html',
  styleUrls: ['./gen-doc-paiement-preinscription.component.scss']
})
export class GenDocPaiementPreinscriptionComponent implements OnInit {
  ...
  exportPDF() {
    ...
    var opt = {
      margin: 0,
      filename: `PAIEMENT_PREINSCRIPTION_${this.user.lastname}_${this.user.firstname}.pdf`,
  ...
  saveDocument() {
    ...
    this.GenDocService.create(new DocumentInternational(null, this.generateCode(), data, this.ecole_id, "Paiement Préinscription", new Date(), this.token.id, `PAIEMENT_PREINSCRIPTION_${this.user.lastname}_${this.user.firstname}.pdf`)).subscribe(doc => {
      ...
      html2pdf().from(element).set(opt).toPdf()
        ...
        .then(pdfBase64 => {
          ...
          const file = new File(
            [pdfBase64],
            `PAIEMENT_PREINSCRIPTION_${this.user.lastname}_${this.user.firstname}.pdf`,
