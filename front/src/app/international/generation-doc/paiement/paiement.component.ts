...
export class PaiementComponent implements OnInit {
  ...
  exportPDF() {
    ...
    var opt = {
      margin: 0,
      filename: `PAIEMENT_${this.user.lastname}_${this.user.firstname}.pdf`,
  ...
  saveDocument() {
    ...
    this.GenDocService.create(new DocumentInternational(null, this.generateCode(), data, this.ecole_id, "Paiement", new Date(), this.token.id, `PAIEMENT_${this.user.lastname}_${this.user.firstname}.pdf`)).subscribe(doc => {
      ...
      html2pdf().from(element).set(opt).toPdf()
        ...
        .then(pdfBase64 => {
          ...
          const file = new File(
            [pdfBase64],
            `PAIEMENT_${this.user.lastname}_${this.user.firstname}.pdf`,
