...
@Component({
  selector: 'app-gen-doc-paiement-acompte',
  templateUrl: './gen-doc-paiement-acompte.component.html',
  styleUrls: ['./gen-doc-paiement-acompte.component.scss']
})
export class GenDocPaiementAcompteComponent implements OnInit {
  ...
  exportPDF() {
    ...
    var opt = {
      margin: 0,
      filename: `PAIEMENT_ACOMPTE_${this.user.lastname}_${this.user.firstname}.pdf`,
  ...
  saveDocument() {
    ...
    this.GenDocService.create(new DocumentInternational(null, this.generateCode(), data, this.ecole_id, "Paiement Acompte", new Date(), this.token.id, `PAIEMENT_ACOMPTE_${this.user.lastname}_${this.user.firstname}.pdf`)).subscribe(doc => {
      ...
      html2pdf().from(element).set(opt).toPdf()
        ...
        .then(pdfBase64 => {
          ...
          const file = new File(
            [pdfBase64],
            `PAIEMENT_ACOMPTE_${this.user.lastname}_${this.user.firstname}.pdf`,
