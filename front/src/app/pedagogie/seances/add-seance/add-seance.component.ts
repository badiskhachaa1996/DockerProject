...
export class AddSeanceComponent implements OnInit {
  ...
  saveSeance() {
    ...
    if(!this.seanceForm.get('isUnique').value)
    {
      ...
      let daysBetween = this.getDaysBetweenTwoDates(dateDebut, dateFinPlan);
      ...
      for(let i = 1; i <= daysBetween; i++)
      {
        ...
        if(newDateDebut.getDay() == parseInt('0') || newDateDebut.getDay() == parseInt('6'))
        {
          console.log('week ' + newDateDebut.getDay());
        }
        else 
        {
          newNumeroSeance +=  1;
