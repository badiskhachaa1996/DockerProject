...
export class GestionTicketsComponent implements OnInit {
  ...
  get description() { return this.TicketFormAdd.get('description'); }
  ...
  endDateOfTreatment(rawData) {
    let calc = new Date(new Date(rawData.date_ajout).getTime() - new Date(rawData.date_fin_traitement).getTime())
    let days = (calc.getUTCDate() - 1 > 0) ? "" + (calc.getUTCDate() - 1) + " j " : " ";
    let Hours = calc.getUTCHours();
    let minutes = calc.getUTCMinutes();
    return days.toString() + Hours + " h " + minutes + " min "
