...
@Component({
  selector: 'app-entreprises-missions',
  templateUrl: './entreprises-missions.component.html',
  styleUrls: ['./entreprises-missions.component.scss']
})
export class EntreprisesMissionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getInfoById(this.token.id).subscribe(
      ((userFromDb) => { 
        this.userConnected = userFromDb; 
        if(this.userConnected.type == 'CEO Entreprise')
        {
          this.entrepriseService.getByDirecteurId(this.userConnected._id).subscribe(
            ((entrepriseFromDb) => { 
              this.userEntreprise = entrepriseFromDb; 
              this.userType = userFromDb;
            }),
        ...
        {
          this.tuteurService.getByUserId(this.userConnected._id).subscribe(
            ((tuteurFromDb) => { 
              this.userType = tuteurFromDb ;
              this.entrepriseService.getByIdTuteur(this.userType._id).subscribe(
                ((entrepriseFromDb) => { this.userEntreprise = entrepriseFromDb; }),
                ((error) => { console.log(error); })
              );
            }),
            ((error) => { console.log(error);})
        ...
        this.missionService.getMissionsByEntrepriseId(this.userEntreprise._id)
  ...
  onAddMission(): void 
  {
    ...
    mission.entreprise_id   = this.userEntreprise._id;
